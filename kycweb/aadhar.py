...
def get_labels_from_aadhar(temp):
    ...
    #imp['Aadhar No'] = re.findall(r"[0-9]{4}\s[0-9]{4}\s[0-9]{4}", temp[0])
    #if temp[1].endswith("Female") or temp[1].endswith("FEMALE"):
    #    imp["Gender"] = "Female"
    #elif temp[1].endswith("Male") or temp[1].endswith("MALE"):
    #    imp["Gender"] = "Male"
    
    #imp['DOB'] = re.search(r'\b\d{2}/\d{2}/\d{4}$', temp[2])[0]
    #imp['Name'] = temp[3]
    ...
    for idx in range(len(temp)):
        ...
        try:
            ...
            if re.search(r"[0-9]{4}\s[0-9]{4}\s[0-9]{4}", temp[idx]):
                try:
                    imp['AadharNo'] = re.findall(r"[0-9]{4}\s[0-9]{4}\s[0-9]{4}", temp[idx])[0]
                except Exception as _:
                    imp['AadharNo'] = "Not Found"
                ...
                try:
                    imp["DateofBirth"] = re.findall(r"[0-9]{2}\-[0-9]{2}\-[0-9]{4}", temp[idx])[0]
                except Exception as _:
                    imp["DateofBirth"] = re.findall(r"[0-9]{2}/[0-9]{2}/[0-9]{4}", temp[idx])[0]
                imp["Name"] = temp[idx + 1]
            ...
            elif imp["DateofBirth"] == None and re.search(r"[0-9]{4}", temp[idx]):
                # handle exception if Year of Birth is not found but string similar to year is found
                try:
                    imp["YearofBirth"] = re.findall(r"[0-9]{4}", temp[idx])[0]
                except Exception as _:
                    imp["YearofBirth"] = "Not Found"
                imp["Name"] = temp[idx + 1]
