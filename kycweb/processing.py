...
import re
from kycweb.ctpn.demo_pb import get_coords
import numpy as np
...
def recognise_text(image_path, photo_path, orig_path, sign=""):
    ...
    for coords in coordinates:
        ...
        # get text from the image, lang = english, config = use lstm for
        # prediction
        ...
        # ignore text if the length of text is less than 3 as it would only be
        # noise
        if len(text) < 3:
            continue
    ...
    face, found = "Photo not found!", False #get_photo(orig_path)
